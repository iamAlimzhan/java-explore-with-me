...
public interface RequestRepository extends JpaRepository<ParticipationRequest, Long> {
    ...
    @Query("select new ru.practicum.model.Request(r.event.id,COUNT(distinct r)) " +
            "FROM requests r " +
            "where r.status = 'CONFIRMED' and r.event.id IN :eventsIds group by r.event.id")
    List<VerifyRequest> findConfirmedRequest(@Param(value = "eventsIds") List<Long> eventsIds);
